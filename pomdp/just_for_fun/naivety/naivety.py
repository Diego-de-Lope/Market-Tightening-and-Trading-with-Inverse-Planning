...
PLANNING_BETA_C = .025 # WE SET THIS TO BE ASSUMED AS RATIONAL, EVEN THOUGH TRADER C IS IRRATIONAL
        ...
        for step in range(30):
            ...
            if curr_turn == 0:
                ...
                if curr_turn == 1: # TRADER B (Rational)
                    real_beta = 20.0        # B is actually smart
                    assumed_beta = 20.0     # A knows B is smart
                    source_label = "Rational"
                else:              # TRADER C (Irrational)
                    real_beta = 0.025         # REALITY: C is always Noisy/Irrational
                    assumed_beta = PLANNING_BETA_C # MODEL: A uses the Global Setting
                    source_label = "Irrational"

                # 1. ACTION (The Truth): Use 'real_beta'
                action = int(get_stochastic_action(
                    b_idx, a_idx, mu_i,
                    real_beta,
                    subkey
                ))
                source = f"{source_label} (Mu={mu_i}, RealBeta={real_beta})"

                # 2. SENSOR (The Perception): Use 'assumed_beta'
