...
# --- TRUE VALUES USED FOR SCORING (CONSISTENT WITH hidden_mu_vals) ---
TRUE_MU_BY_TRADER = {
    "A": float(hidden_mu_vals[0]),
    "B": float(hidden_mu_vals[1]),
    "C": float(hidden_mu_vals[2]),
}
...
def get_utility(bi_next, ai_next, ti_next, a_cand, future_q_val, b_idx, c_idx):
    ...
    relevant_mu = np.where(
        ti_next == 1, belief_mu_vals[b_idx],
...
def run_simulation(Q_vals, visualize=True):
    ...
    for round_num in range(1, 4):
        ...
        for step in range(30):
            ...
            # ==========================
            # EXECUTION AND SCORING FIX
            # ==========================
            if action == 1:  # BUY at ask
                mu_actor_true = TRUE_MU_BY_TRADER[trader]
                mu_prev_true = TRUE_MU_BY_TRADER[prev_trader]

                r_actor = mu_actor_true - curr_ask
                r_prev = curr_ask - mu_prev_true

                mu_actor_hidden = float(hidden_mu_vals[curr_turn])
                mu_prev_hidden = float(hidden_mu_vals[prev_trader_idx])

                r_actor_hidden = mu_actor_hidden - curr_ask
                r_prev_hidden = curr_ask - mu_prev_hidden

                total_true_rewards[trader] += r_actor
                total_true_rewards[prev_trader] += r_prev
                total_hidden_rewards[trader] += r_actor_hidden
                total_hidden_rewards[prev_trader] += r_prev_hidden

                print(f" >>> EXECUTION: {trader} BUYS from {prev_trader} @ {curr_ask}")
                print(f"     Round Profits: {trader}={r_actor}, {prev_trader}={r_prev}")

                curr_turn = (curr_turn + 1) % 3
                history_data.append({'round_num': round_num, 'steps': round_steps})
                break

            elif action == 2:  # SELL at bid
                mu_actor_true = TRUE_MU_BY_TRADER[trader]
                mu_prev_true = TRUE_MU_BY_TRADER[prev_trader]

                r_actor = curr_bid - mu_actor_true
                r_prev = mu_prev_true - curr_bid

                mu_actor_hidden = float(hidden_mu_vals[curr_turn])
                mu_prev_hidden = float(hidden_mu_vals[prev_trader_idx])

                r_actor_hidden = curr_bid - mu_actor_hidden
                r_prev_hidden = mu_prev_hidden - curr_bid
                ...
                if (curr_bid + 1) < (curr_ask - 1):
                    ...
                    if no_progress_count >= 3:
                        print("     Market Locked. Round Void.")
                        history_data.append({'round_num': round_num, 'steps': round_steps})
                        break
